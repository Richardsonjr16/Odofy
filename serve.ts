...
const freeBackendPort =
  `for _ in $(seq 1 25); do ` +
  `pids=$(lsof -t -iTCP:${String(BACKEND_PORT)} -sTCP:LISTEN 2>/dev/null || true); ` +
  `if [ -z "$pids" ]; then exit 0; fi; ` +
  `kill $pids 2>/dev/null || true; sleep 0.2; ` +
  `done`;

await Bun.$`sudo sh -c ${freeBackendPort}`.quiet().nothrow();

const backendProcess = Bun.spawn(["node", "src/server.js"], {
  cwd: "/home/team/shared/odofy-backend",
  stdout: "inherit",
  stderr: "inherit",
  onExit: (_proc, exitCode, signalCode, _error) => {
    console.error(`[serve] Backend exited: code=${exitCode} signal=${signalCode}`);
  },
});

...
const marketThemeDirectory: Record<
  string,
...
function SlideTrack({
  ...
  const handleMove = useCallback(
    (clientX: number) => {
      ...
      const raw = ((clientX - rect.left - thumbWidth / 2) / maxTravel) * 100;
...
function GeofenceMiniMap({
  ...
  return <div ref={miniRef} className="w-full h-full" />;
}
...
function DashboardPage() {
  ...
  const [offerEndTime, setOfferEndTime] = useState<string | null>(
    () =>
      (typeof window !== "undefined"
        ? sessionStorage.getItem("odofy_offer_end_time")
        : null) || null,
  );
  ...
  const [isOdofyNowActive, setIsOdofyNowActive] = useState(
    () =>
      (typeof window !== "undefined"
        ? sessionStorage.getItem("odofy_now_active")
        : null) === "true",
  );
  ...
  useEffect(() => {
    if (!scannerOpen || !claimedTrip || !token) return;
    ...
    scanner
      .start(
        ...
        async (decodedText) => {
          ...
          try {
            await scanner.stop();
            const res = await fetch("/api/v1/orders/verify-qr-handshake", {
              method: "POST",
              headers: {
            ...
            if (!res.ok) {
              ...
              throw new Error(body.error || "Invalid QR code");
            }
            setScannerOpen(false);
            setClaimedTrip((current) =>
  ...
  const currentMarketColors =
    marketThemeDirectory[activeCityHub] || marketThemeDirectory["springfield"];
  ...
  const mapMarkers = useMemo(() => {
    ...
    if (claimedTrip && activeDeliveryStep === "EN_ROUTE") {
      markers.push({
        lat: isReturning
          ? Number(claimedTrip.merchant_lat)
          : Number(claimedTrip.dest_latitude),
        lng: isReturning
          ? Number(claimedTrip.merchant_lng)
  ...
  const handleSlideArrivalComplete = async () => {
    ...
    setScannerOpen(true);
    return;
    /* Legacy status transition is intentionally unreachable: QR handshake is required. */
    try {
      const res = await fetch(
        `/api/v1/odofy/trips/${claimedTrip.uuid}/status`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleConfirmRestock = async () => {
    ...
    try {
      ...
      setClaimedTrip((current) =>
        current ? { ...current, driver_payout: "12.75" } : current,
      );
      setClaimedTrip(null);
      setActiveDeliveryStep("IDLE");
  ...
  const handleNavigate = () => {
    if (!claimedTrip) return;
    const latitude = isReturning
      ? claimedTrip.merchant_lat
      : claimedTrip.dest_latitude;
    const longitude = isReturning
      ? claimedTrip.merchant_lng
      : claimedTrip.dest_longitude;
      ...
      <div className="min-h-dvh bg-[#F8F9FA] flex flex-col items-center justify-center px-4">
        <div className="w-full max-w-sm">
          ...
          <form
            ...
            style={
              {
                "--odofy-p": currentMarketColors.primary,
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                boxShadow: `0 4px 6px -1px ${hexToRgba(currentMarketColors.primary, 0.2)}`,
              }}
            >
              Sign In
            </button>
  ...
  if (activeDeliveryStep === "EN_ROUTE" && claimedTrip) {
    ...
    const tripDistance = currentLocation
      ...
      : 4.3;
    const withinMerchantGeofence = Boolean(
      currentLocation &&
      ...
      <div className="max-w-md mx-auto min-h-screen flex flex-col font-sans relative overflow-hidden bg-white">
        ...
        <div className="bg-white rounded-t-[28px] -mt-4 px-4 pt-6 pb-4 flex flex-col gap-4 z-20 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
          ...
          {crossStackBonus > 0 && (
            <div className="flex flex-wrap gap-2">
              <span className="bg-[#E6F4EA] text-[#137333] px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                Multi-trip incentive: +${crossStackBonus.toFixed(2)} cross-stack
                bonus included
              </span>
            </div>
          )}
          ...
          <div className="flex gap-4 justify-center">
            <button
              onClick={() =>
                window.open(`tel:${claimedTrip.customer_phone || ""}`, "_self")
              }
          ...
          {restockSuccess && (
            <p className="text-center text-sm font-bold text-green-700">
              Return restock confirmed — payout: $12.75.
            </p>
          )}
  ...
  const submitIdentityCheck = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(data.error || "Unable to submit identity check");
      ...
      setIdentityError(
        error instanceof Error
          ? error.message
          : "Unable to submit identity check",
      );
    } finally {
          ...
          <div className="bg-white max-w-md w-full rounded-2xl p-6 shadow-2xl border border-gray-100 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 text-sm text-gray-700">
              <p>
                ...
                from the dropdown menu to start driving.
              </p>
              <p>
                🥇 2. JUST FOR YOU: You get 60 secs to claim exclusive offers
                before they blast out to the general fleet.
              </p>
              <p>
                📦 3. ONE-TAP PICKUP: Match the bag ID tag at the store counter.
                No need to check individual grocery items.
              </p>
              <p>
                🛑 4. GEOCONTROLS: You must be inside a strict 150-ft radius of
                BOTH the store and the customer doorstep to tap check-in actions
                or submit photos.
              </p>
            </div>
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
            <div className="mb-5 text-center">
              ...
              <p className="mt-2 text-sm leading-relaxed text-gray-600">
                Identity Check: Take a clear Front, Left, and Right view selfie
                to verify your driver profile.
              </p>
            </div>
                ...
                <label key={key} className="cursor-pointer text-center">
                  ...
                  <div className="flex aspect-square items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-[#5E0009]/30 bg-gray-50">
                    {identityFiles[key] ? (
                      ...
                      <span className="text-2xl text-[#5E0009]">＋</span>
                    )}
                  </div>
                  <input
                    type="file"
      ...
      {isTooEarlyModalOpen &&
        (() => {
          ...
          const estimatedArrival = new Date(
            now.getTime() + travelTimeMins * 60000,
          );
          const targetPickup = claimedTrip?.scheduled_pickup_time
            ? new Date(claimedTrip.scheduled_pickup_time)
            : null;
            ...
            <div className="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex flex-col justify-end animate-fade-in">
              <div className="bg-white rounded-t-3xl p-6 w-full max-w-md mx-auto flex flex-col items-start gap-4 pb-8 border-t border-gray-100 shadow-2xl animate-slide-up">
                ...
                <p className="text-sm font-medium text-gray-500 leading-relaxed mt-2">
                  This business isn't expecting you until the pickup time. You
                  can start the trip when your ETA is closer to{" "}
                  {formatPickupTime(targetPickup)}.
                </p>
                <button
          ...
          <div className="w-full h-full bg-gray-50 overflow-y-auto px-4 py-6">
            ...
            {trips.filter((t) => !removedIds.has(t.uuid)).length === 0 ? (
              ...
              <div className="space-y-3">
                {trips
                  .filter((t) => !removedIds.has(t.uuid))
                  .map((trip) => (
                    <div
                      key={trip.uuid}
      ...
      {scannerOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="w-full max-w-sm mx-auto bg-black rounded-2xl overflow-hidden relative border-4 border-[#5E0009]">
            ...
            <p className="text-white text-center text-xs py-3">
              Point camera at customer's QR code
            </p>
            {scannerError && (
              <p className="text-red-300 text-center text-xs pb-3 px-3">
                {scannerError}
              </p>
            )}
          </div>
            ...
            <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 animate-fade-in">
              <div className="bg-white rounded-t-[28px] sm:rounded-2xl w-full max-w-md mx-auto flex flex-col items-center gap-3 px-6 pt-8 pb-6 shadow-2xl animate-slide-up max-h-[90vh] overflow-y-auto">
                ...
                <p className="text-sm font-medium text-gray-600 text-center px-2 leading-relaxed">
                  Once you're within the 150-foot radius circle, you can try
                  confirming your arrival again.
                </p>
      ...
      {isTimeDrawerOpen && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white rounded-t-[28px] z-50 px-6 pt-6 pb-8 animate-slide-up shadow-[0_-10px_40px_rgba(0,0,0,0.15)]">
            ...
            <div
              className="mb-6 overflow-y-auto rounded-2xl border border-gray-100"
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor: isSelected
                        ? hexToRgba(currentMarketColors.primary, 0.08)
                        : "transparent",
                    }}
                  >
                    <span
                    ...
                    <button
                      ...
                      style={
                        isActive
                          ? {
                              backgroundColor: currentMarketColors.primary,
                              borderColor: currentMarketColors.primary,
                            }
                          : undefined
                      }
                    >
